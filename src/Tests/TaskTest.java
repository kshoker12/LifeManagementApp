...
public class TaskTest {
    ...
    void testAddSubTask() {
        assertEquals(task.getSubTasks().size(), 0);
        SubTask subTask = new SubTask("Sprints", 1, task.getName());
    ...
    void testRemoveSubTask() {
        ...
        task.addSubtask(subTask);
        assertEquals(task.getSubTasks().size(), 1);
        task.removeSubtask(new SubTask("Sprints", 1, task.getName()));
        assertEquals(task.getSubTasks().size(), 0);
    }
    ...
    void testIsComplete() {
        SubTask subTask = new SubTask("Sprints", 1, task.getName());
        SubTask subTask1 = new SubTask("Box", 1, task.getName());
        task.addSubtask(subTask);
